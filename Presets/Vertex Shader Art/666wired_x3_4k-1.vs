/*{
  "DESCRIPTION": "666wired x3 4k",
  "CREDIT": "anon (ported from https://www.vertexshaderart.com/art/zGGRYEyPC9E6CWEWD)",
  "ISFVSN": "2",
...
//vec3 colour = vec3(parameter6* .003)
  ...
  for (float i = 0.; i < 8.; i++) {
   ...
   lightSource.yz *= m;
    ...
    float snd = max(0.,texture(sound, vec2(float(i)/float(LIGHTS)*0.2+parameter7, 0.)).r - texture(sound, vec2(float(i)/float(LIGHTS)*.5+.03, 0.0)).r)*5.
