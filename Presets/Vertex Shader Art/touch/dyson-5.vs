/*{
  "DESCRIPTION": "dyson",
  "CREDIT": "valentin (ported from https://www.vertexshaderart.com/art/q9prYaawf95rZ4Bas)",
  "ISFVSN": "2",
  ...
  "INPUTS": [ { "LABEL": "Touch", "NAME": "touch", "TYPE": "image" }, { "LABEL": "Background", "NAME": "background", "TYPE": "color", "DEFAULT": [ 0.25, 0.59, 0.9, 1.0 ] }, { "LABEL": "Sound", "NAME": "sound", "TYPE": "audioHistogram" } ],
  "METADATA": {
    "ORIGINAL_VIEWS": 2,
    "ORIGINAL_DATE": {
      "$date": 1508053618683
...
void cube(const float vertexId, const float vertexCount, const float base, const mat4 cmat, const mat4 vmat, out vec3 pos, out vec4 color) {
  ...
  float s1 = texture(sound, vec2(mix(0.01, 0.001, abs(t.x)), abs(t.x) * 1.)).r;
  float s2 = texture(sound, vec2(mix(0.01, 0.001, abs(t.y)), (1. - abs(t.y)) * 1.)).r;
  float s3 = texture(sound, vec2(mix(0.01, 0.001, abs(t.z)), abs(t.z) * 1.)).r;
