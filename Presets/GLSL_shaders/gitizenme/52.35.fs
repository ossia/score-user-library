...
vec4 map( in vec3 pos, float atime )
{
	...
	href = deli1Pos.y;
    float deli1 = sdEllipsoid(deli1Pos, scale * vec3(2.1, 1.2, 1.25));
    ...
    xx = r.x - 0.2 - .5 * r.z * r.z;
    yy = r.y - 0.2 - .5 * r.x * r.x;
    w = 0.009 * cos(yy * wrinkles) * sin(xx * wrinkles) * (2.0 - smoothstep(0.1, 0.9, abs(yy)));
    vec3 deli3Pos = r - vec3(w + 1., elevation + fh * 2., -2); // - cen;
    deli3Pos.xy *= rotate(sin(TIME));
    deli3Pos.xz *= rotate(sin(TIME));
    float deli3 = sdEllipsoid(deli3Pos, scale * vec3(2, 1.5, 0.5));
