/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#42405.8"
}
*/
...
void main( void ) {
	...
	//noise1
	vec3 vv = snoiseVec3(vec3(floor(position.x * v1)/v1, floor(position.y * v1)/v1, (TIME*0.4)*speed_back));
	vv.x = 0.5 + vv.x * 0.5;
	vv.y = 0.3 + vv.y * 0.5;
	vv.z = 0.8 + vv.z * 0.2;
			
	//noise2
	vv += vec3(0.05 * rand(position * 10.0 + (TIME)));
