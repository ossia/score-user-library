/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#56156.0"
}
*/
...
vec3 draw (vec3 p,float t){
	float d = length(p.xz);
	float y = (fract(d * 100. - 0.05) - 0.05)/fwidth(d * 100.);
	return vec3(1.0 - min(y, 1.0)) * (1.0-step(8., t));
}
