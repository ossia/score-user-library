/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#3574.0"
}
*/
...
void main( void ) {
	...
	for(int i=0; i<80; i++)
	{
		...
		p+=vec2(-float(i-30)*0.005,float(i-30)*0.008);
		
		
		vec2 c = vec2((p.x-0.5)/p.y,1.0/(p.y+0.2))+mouse*vec2(-0.5,0.5);
	...
	vec3 col =mix(vec3(01.6/(position.y*2.5+0.2),0.9/(position.y*9.5+0.1),0.2)*0.5,d,clamp(0.0,q*1.9,1.0));
