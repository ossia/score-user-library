  ...
  "INPUTS" : [
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#35188.1"
}
*/
...
float distance_field(vec3 p) {
	...
	return mix(a, b, .5+0.25*sin(((TIME*(speed/5.))+length(p.yz))*12.));
}
...
void main( void ) {
	...
	mat3 trans = make_y_rot(TIME*speed)*make_x_rot(TIME*(speed/2.));
	...
	gl_FragColor = c*vec4(1.0, 1.0, 1.0, 4.0);
}