/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#57034.0"
}
*/
...
{ fragColor = vec4(0., 0., 0., 1.);
   ...
   //uv/=8.;
    ...
    if(mod(iTime, 7.)>6.){okgreen = goodred;}
    else if(mod(iTime, 7.)>5.){okgreen = goodpurple;}
    else if(mod(iTime, 7.)>4.){okgreen = goodblue;}
    else if(mod(iTime, 7.)>3.){okgreen = okgreen;}
    else if(mod(iTime, 7.)>2.){okgreen = drkpurple;}
    else if(mod(iTime, 7.)>1.){okgreen = cyan;}
    else {okgreen = brown;}
