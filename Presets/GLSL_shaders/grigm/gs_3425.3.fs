/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#3425.3"
}
*/
...
vec3 mod289(vec3 x)
{
  return x - floor(x * (1.0 / 289.0)) * sin(x);
}
...
float curve( float x)
{
	return abs(smoothstep( 10.0, 0.0,sin(x*PI) ) / abs(unsin(x*PI)+0.0001));
}
...
float thing(vec2 pos) 
{
	...
	float a = curve(pos.x);
	float b = curve(pos.y);
	float c = 1./cnoise(vec3(pos.x, pos.y, TIME*0.1));
