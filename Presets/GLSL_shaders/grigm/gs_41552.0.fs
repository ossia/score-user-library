/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#41552.0"
}
*/
...
vec3 camera (vec3 p) {
    p.yz *= rot((PI*(mouse.y/RENDERSIZE.y-.5)*step(0.5,mouse.x)));
    p.xz *= rot((PI*(mouse.x/RENDERSIZE.x-.5)*step(0.5,mouse.x)));
    p.yz *= rot(-PI*.5);
    return p;
}
