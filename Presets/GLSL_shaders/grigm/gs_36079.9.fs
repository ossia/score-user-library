/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#36079.9"
}
*/
...
void main( void ) {
	...
	col += regularPolygon(4, 0.1, 0.02, vec3(0.0, 1.0, 1.0), vec2(0.5, 0.65), rot, p);
	col += regularPolygon(5, 0.1, 0.02, vec3(1.0, 0.5, 0.0), vec2(0.8, 0.65), rot, p);
	col += regularPolygon(6, 0.1, 0.02, vec3(1.0, 0.0, 1.0), vec2(0.2, 0.35), rot, p);
	col += regularPolygon(7, 0.1, 0.02, vec3(0.5, 0.9, 0.5), vec2(0.5, 0.35), rot, p);
	col += regularPolygon(8, 0.1, 0.02, vec3(0.5, 0.5, 0.9), vec2(0.8, 0.35), rot, p);
