/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#1946.2"
}
*/
...
float surface3 ( vec3 coord ) {
	...
	n += 0.25	* abs( cnoise( coord * frequency * 4.0 ) );
		...
		n += 0.06125	* abs( cnoise( coord * frequency * 16.0 ) );
	n += (0.06125 / 2.0)	* abs( cnoise( coord * frequency * 32.0 ) );
...
void main( void ) {
  ...
  if (uv.x != (vx_offset-0.005))
  {
	  ...
	  //mod(res * surfacePosition.x, 1.) < 0.5 ^^ mod(res * surfacePosition.y, 1.) < 0.5
    float lum = length(n);
    tc = vec3(1.0, 1.0, 1.0);

    if (lum < lum_threshold_1)
    {
      if (mod(0.8 * gl_FragCoord.x , 8.0) < 0.5 ^^ mod(0.8 * gl_FragCoord.y, 8.) < 0.5)
        tc = vec3(0.0, 0.0, 0.0);
	else
		tc = vec3(1.0, 1.0, 1.0);
    ...
    {
      if (mod(0.4 * gl_FragCoord.x , 4.0) < 0.5 ^^ mod(0.4 * gl_FragCoord.y, 4.) < 0.5)
        tc = vec3(0.0, 0.0, 0.0);
	else
		tc = vec3(1.0, 1.0, 1.0);
    }  
    ...
    {
      if (mod(0.2 * gl_FragCoord.x , 2.0) < 0.5 ^^ mod(0.2 * gl_FragCoord.y, 2.) < 0.5)
        tc = vec3(0.0, 0.0, 0.0);
	else
		tc = vec3(1.0, 1.0, 1.0);
    }  
