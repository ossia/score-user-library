/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#60136.1"
}
*/
...
void main(void)
{
  ...
  for (int i = 0; i < maxI; i++) {
    ...
    bool lipsi = ipsoid(vec2(0.2*sin((0.2*TIME+i_ratio)*3.1415)+0.1*sin(2.0+1.5*i_ratio*mouse.x), 0.2*cos(0.1*TIME+123.0)*cos((0.3*TIME+i_ratio*mouse.y)*3.1415)),
                        3.0 - (i_ratio) * 3.0,
                        0.85,
                        3.1415/2.0 + sin(5.0+mouse.x*0.01*(0.09+sin(TIME*0.2))*i_ratio*TIME+TIME) + sin(i_ratio*mouse.x) + 0.25*TIME*mouse.y/100.0,
                        pos);
