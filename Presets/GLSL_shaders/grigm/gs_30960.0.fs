/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#30960.0"
}
*/
...
void main( void ) {
	...
	Ray r;
	r.orign = vec3(0, 0, -4.0 + sin(TIME*0.1));
	r.direction = normalize(vec3(vec2(1, asp)*(tx + noise), 1.0) - r.orign);
	
	mat3 rot = rotation(normalize(vec3(1,sin(TIME),0)), TIME);
	r.orign = rot * r.orign;
	r.direction = rot * r.direction;
	
	vec3 outcol = vec3(0);//envmap(r.direction);
	...
	if(hit)
	{
		...
		float l = isect.TIME;
		outcol = /*vec3(pow(isect.aabb_edge/1.5, 5.0)) +*/  exp(-l*l*0.01)*(fres * envmap(rfl)  + (1.0-fres) * envmap(rfr)) ;//vec3(1,0,0);
	}
	//outcol = vec3(noise,1);
