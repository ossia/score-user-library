/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#5359.8"
}
*/
...
void main( void ) {
	...
	float n = surface3(vec3(position*sin(TIME*LIGHT_PULSE_ZOOM_SPEED), TIME * LIGHT_1_NOISE_SPEED)*mat3(1,sin(1.0),0,0,.8,.6,0,-.6,.8),0.9);
	float n2 = surface3(vec3(position*cos(TIME*LIGHT_2_PULSE_ZOOM_SPEED), TIME * LIGHT_2_NOISE_SPEED)*mat3(1,cos(1.0),0,0,.8,.6,0,-.6,.8),0.8);
    	float lum = length(n);
	...
	vec3 curr_color = (tc*LIGHT_1_FADE) + (tc2*LIGHT_2_FADE);
	...
	for (int i = 0; i < 10; i++) {
		...
		float t = (TIME+angleRnd1*10.)*STAR_ZOOM_SPEED;
	...
	gl_FragColor = vec4(curr_color, 1.0)+vec4( color1,color1,color,color1)/((n+n2)/STARS_FADE);
}