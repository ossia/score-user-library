/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/lsl3RH by iq.  Warp. Tutorial here: ",
  "INPUTS" : [
...
{
    float ql = length( q );
    q.x += 0.05*sin(0.27*TIME+ql*4.1);
    q.y += 0.05*sin(0.23*TIME+ql*4.3);
    q *= 0.5;
	...
	float ol = length( o );
    o.x += 0.02*sin(0.12*TIME+ol)/ol;
    o.y += 0.02*sin(0.14*TIME+ol)/ol;
...
vec3 doMagic(vec2 p)
{
	...
	vec3 nor = normalize( vec3( dFdx(f)*RENDERSIZE.x, 6.0, dFdy(f)*RENDERSIZE.y ) );
