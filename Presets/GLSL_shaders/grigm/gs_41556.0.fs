/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#41556.0",
  "INPUTS" : [
    {
      "NAME" : "mouse",
      "TYPE" : "point2D",
...
vec3 HeadLights(float i, float t) {
    ...
    float size = mix(.03*scale1, .05*scale1, S(.02, .07, z))*d;
    float m = 0.;
    float blur = .1;
...
vec3 StreetLights(float i, float t) {
    ...
    distFade *= (1.-z);
    float m = BokehMask(ro, rd, p, (.05*d)*scale3, blur)*distFade;
...
vec3 EnvironmentLights(float i, float t) {
    ...
    vec3 rp = ClosestPoint(ro, rd, p);
    float distFade = Remap(1., .7, .1, 1.5, 1.-pow(1.-z,6.));
    float m = BokehMask(ro, rd, p, (.05*d)*scale4, blur);
...
void main( void ) {
    ...
    vec2 mouse = mouse.xy/RENDERSIZE.xy;
    
    vec3 pos = vec3(lookAtX, lookAtY, lookAtZ);
    ...
    uv.y += bumps*4.;
    CameraSetup(uv, pos, lookat, camZoom, mouse.x);
