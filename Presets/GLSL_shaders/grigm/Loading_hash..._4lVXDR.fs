/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/4lVXDR by Zavie.  Just an experiment with ray tracing a sphere then rejecting solutions based on a hash and see what happens.",
  "INPUTS" : [
...
Hit intersectHashedSphere(Sphere s, Ray r)
{
    ...
    if (t < 0. || surfaceHash(p.xy + vec2(0., -TIME), 3.) < 0.5)
    {
        t = t2;
        p = r.o + t * r.d;
        n = -(r.o + t*r.d - s.p) / s.r;
...
Hit intersectScene(Ray r)
{
    ...
    vec3 axis2 = randomVector(floor(TIME+1.));
    ...
    Sphere s = Sphere(2.0+sin(TIME+1.), vec3(0., 1.6, 0.), weirdMat);
    Plane p  = Plane(0., vec3(0., 1., 0.), Material(vec3(0.05), vec3(0.02)));

    Hit hit = Hit(1e5, vec3(0.), Material(vec3(-1.), vec3(-1.)));
    Hit hitp = intersectPlane(p, r);  if (hitp.m.f0.r >= 0. && hitp.t < hit.t) { hit = hitp; }
    Hit hits = intersectHashedSphere(s, r); if (hits.m.f0.r >= 0. && hits.t < hit.t) { hit = hits; }
    return hit;
}
...
void main()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        vec3 p = p0;
        vec3 offset = vec3(msaa[i] / RENDERSIZE.y, 0.);
        vec3 d = normalize(vec3(RENDERSIZE.x/RENDERSIZE.y * uv.x, uv.y, -1.5) + offset);
        Ray r = Ray(p, d);
        color += radiance(r) / 4.;
    }
