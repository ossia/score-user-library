/*
{
  ...
  "DESCRIPTION" : "Automatically converted from http:\/\/glslsandbox.com\/e#50731.1"
}
*/
...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (0.5, 0.75, 1.5);        // character color (R,G,B)
...
float dseg(vec2 p0, vec2 p1)
{
	...
	vec2 cp = (uv - ch_pos - p0) * mat2(dir.x*sin(TIME), dir.y,-dir.y, dir.x);
	return distance(cp*sin(TIME)*2.1, clamp(cp, vec2(0), vec2(distance(p0, p1), .1)));   
}
