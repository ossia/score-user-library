...
float random (in vec2 st) { 
    return fract(sin(dot(st.xy,
                         vec2(randomAmt.x * 100.,randomAmt.y * 100.))) 
                * randomMultiply);
}
...
mat2 rotate2d(float angle){
    return mat2(cos(angle *xyNoiseFactor.x * 100.),-sin(angle),
                sin(angle * xyNoiseFactor.y * 100.),cos(angle));
}
...
void main() {
    ...
    vec2 pos = st.yx*vec2(xyStretch * 100.);
